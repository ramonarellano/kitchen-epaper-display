...
- **Image Transfer via UART:**
  ...
  - **Current diagnostic build (fw=FULL_REINIT_v4).** `TurnOnDisplay()` now sends `POWER_ON` immediately before `DISPLAY_REFRESH` again. That restores two real ~31s refreshes after boot, but later cycles still regress to `DISPLAY_DONE rc=-2` with `refresh=2010`. Added `EPD_BUSY_CMDS cmd04=A->B cmd12=C->D` plus richer `POWER_ON_PRE` logging to see whether `0x04` and `0x12` actually pull BUSY LOW on failing cycles.
