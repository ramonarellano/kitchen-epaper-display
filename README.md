...
- **Image Transfer via UART:**
  ...
  - **Current diagnostic build (fw=FULL_REINIT_v4_5MIN).** Keeps the `EPD_BUSY_CMDS` / `POWER_ON_PRE` instrumentation from v4 and shortens the request interval from 15 minutes to 5 minutes. This isolates whether Bug #15 depends more on idle time between refreshes than on the number of refresh cycles.
