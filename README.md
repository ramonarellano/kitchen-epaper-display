...
- **Image Transfer via UART:**
  ...
  - **Fix: abort-on-timeout + retry (fw=FULL_REINIT_v2).** v1 forced through stuck BUSY (360s), corrupting the panel's state machine. v2 aborts immediately when a phase times out, uses per-phase timeouts (10s/60s/10s), retries Init+PowerOn up to 3 times with fresh hardware resets, and fixes REFRESH_VERDICT to not falsely report timeouts as real refreshes.
  - Added firmware version tag (`fw=FULL_REINIT_v2`) to the BOOT log line for deployment verification.
