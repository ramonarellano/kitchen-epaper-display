...
- **Image Transfer via UART:**
  ...
  - **Fix: abort-on-timeout + retry + WaitBusyTransition (fw=FULL_REINIT_v3).** v2 showed BUSY staying HIGH after reset — ReadBusyH returned instantly (0ms) causing 447ms bogus refreshes even though rc=0. v3 uses `WaitBusyTransition` which requires BUSY to go LOW first (panel acknowledges command), then HIGH (panel finishes). Also samples BUSY 2ms after Reset to detect non-responsive panels. Each command (POWER_ON, REFRESH, Init) now detects if the panel didn't react at all (rc=-1) vs timed out during processing (rc=-2).
